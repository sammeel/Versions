...
const getState = async (pipelines: number[]): Promise<IOverviewTabState> => {
  await SDK.ready();
  ...
  if (project) {
    ...
    const buildDefinitions = await pipelinesClient.getDefinitions(
      project.id,
      undefined,
      undefined,
      undefined,
...
export function VersionsHubContentRelease(props: any) {
  ...
  useEffect(() => {
    const getDataWrapper = async () => {
      ...
      const response = await getState(pipelines);
      setState(response);
    };
    getDataWrapper();
  }, [pipelines]);
